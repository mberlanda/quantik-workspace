...
1. **The app is vendored into `quantik-models` as package data.** Chosen over the
...
2. **The vendored copy is a byte copy, not a build product.** If the app ever
   needs a build step, that is a new decision requiring its own record — not
   something to introduce inside the sync script.

3. **The sync is release-time, not per-PR.** The vendored app lags the visualizer's
...
4. **No skill-level ladder ships here.** Carried forward from QW-010 decision 4
...
2. **What does the app show when the roster is classical-only** — a service with
...
3. **Does the public deployment ship `best` or `full`?** Carried from QW-009
