...
**Publishing: `quantik-models` is NOT on PyPI** (verified 2026-08-30 — `pypi.org/pypi/quantik-models/json` returns 404; only `quantik-core` is published). Trained checkpoints are published separately, as safetensors + ONNX model repositories on Hugging Face under `brpoplpush` — `quantik-cpool-c191-b6`, `quantik-attn-d192-b6`, `quantik-resnet-c128-b6`, `quantik-mlp-h455-b4`, all four public and verified 2026-08-30; the size suffix is part of the repo id. Weights are CC BY-NC 4.0, the code MIT. The full table and the licence reasoning are in [`../system/repository-map.md`](../system/repository-map.md) — see the publishing entry point `python -m quantik_models.export.huggingface` and `scripts/stage_hub_repos.sh`; nothing in the tooling itself uploads.
